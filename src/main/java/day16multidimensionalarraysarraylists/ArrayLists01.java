...
soyledigimiz eleman sayisindan fazla eleman koyamayiz. Array'ler eleman sayisinda "fixed" dirler.
...
2)Array'lerin icine "primitive" ve "reference" lar konabilir.
  ArrayList'lerin icine ise sadece "non-primitive" ler konulur.( wrapper class olarak yazabiliriz)
...
Note: Eleman sayisi kesin belli olan coklu datalari depolamak icin Array kullaniriz, ama
 eleman sayisi degisken olan coklu datalar icin Array kullanma riskini almayin
     ...
     */
        ...
        //2.Yol:**çok kullanılan.özel list gibi (kırmızı ayakkabı)
        ArrayList<Integer> heights=new ArrayList<>();
        //3.Yol:***çok kullanılan.(ayakkabı)
        ...
        nums.add(1,50); //istediğimiz yere eklemek için
        System.out.println(nums);   // [21, 50, 18, 20]

        List<Integer> prices=new ArrayList<>();
        prices.add(23);
        prices.add(185);
        nums.addAll(prices); //listenin içine liste ekledik
...
//Example 1: nums ArrayList'indeki tum tek sayilari 11 artirdiktan sonra ekrana yazdiriniz
        ...
        for (int w:nums){
            if (w%2!=0){  //21->1-> true
                nums.set(nums.indexOf(w),w+11); // teklilerin indeksini girip değiştiriyoruz (update)
            }           //set(0,32)-> ilk indeksteki değeri 11 arttırıp değiştirdik
        }
        ...
        Integer sayi=200;  // 200ü direk silmek için (indeksini bilmiyorsak)
        nums.remove(sayi);
      //  nums.remove(3); ->indeks olarak çıkarma
